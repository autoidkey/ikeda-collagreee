  function treeHJSON(dataAll,idm,title) {
    $( "#dialog" ).dialog({
      autoOpen: false,
      minWidth: 800,
      minHeight:400,
...
var graph = {"graph":[],"links":[],"nodes":[],"directed": false,"multigraph": false}
graph["nodes"].push({ id:0, label:title,node_type:0,node_size:100 })
var doc_details = {0 :{body:title}}
console.log(graph)

// 正規化する
var max_idm = 0
for (var i = 0, len = idm.length; i < len; i++) {
  if (idm[i][1] >max_idm){
    max_idm = idm[i][1]
  }
}
...
for (var i = 0, len = idm.length; i < len; i++) {
  graph["links"].push({source:0 , target:i+1 , distance:2.5})
  var size = 80 * (idm[i][1]/max_idm) 
  if(size < 30) {
    size = 30
  }
  graph["nodes"].push({ id:i+1, label:idm[i][0],node_type:1,node_size:size })
}
